...
class FilterComparison implements FromView
{

    public function __construct($polda, $year, $id_rencana_operasi, $tanggal_operasi)
    {
        $this->polda = $polda;
        $this->year = $year;
        $this->id_rencana_operasi = $id_rencana_operasi;
        $this->tanggal_operasi = $tanggal_operasi;
    }

    public function view(): View
    {

        $thisYear = DailyInput::where('year', $this->year)->where('rencana_operasi_id', $this->id_rencana_operasi)->first();
        $rencana_operasi = RencanaOperasi::where('id', $this->id_rencana_operasi)->first();
        ...
        if(!empty($thisYear)) {
            ...
            ->where('rencana_operasi_id', $this->id_rencana_operasi)
            ->when($this->polda != 'polda_all', function ($q) {
                return $q->where('polda_id', $this->polda);
            })
            ->when($this->tanggal_operasi != 'semua_hari', function ($q) {
                return $q->where(DB::raw('DATE(created_at)'), $this->tanggal_operasi);
            })
            ->first();
        ...
        return view('exports.report_daily_custom', [
            'tahunPertama' => $tahunPertama,
            'tahunKedua' => $tahunKedua,
            'startDate' => $this->tanggal_operasi,
            'endDate' => '',
            'polda_name' => 'TEST POLDA',
